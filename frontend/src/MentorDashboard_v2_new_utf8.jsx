...
function MentorDashboard({ mentorAuth, onLogout }) {
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            ...
            if (res.ok) {
                showToast('Profile updated successfully! Γ£¿');
    ...
    useEffect(() => {
        if (mentorId && showAnalytics) {
            fetch(`${API_BASE_URL}/api/mentors/${mentorId}/analytics/views`)
                .then(res => res.json())
                .then(data => setDailyViews(data))
                .catch(err => console.error("Error fetching views:", err));
        }
    }, [mentorId, showAnalytics]);
    ...
    const todayReceived = bookings.filter(b => {
        try {
            ...
            <header className="header">
                <div className="header-content">
                    ...
                    <div className="header-badge" style={{cursor: 'default', left: '50%'}}>
                        ≡ƒöÑ {todayReceived} Requests Received Today
                    </div>
                    ...
                    <nav>
                        <ul className="nav-links">
                            ...
                            <li>
                                <a href="#" 
                                   className={showAnalytics ? 'active-nav' : ''} 
                                   onClick={(e) => { 
                                       e.preventDefault(); 
            ...
            <div style={{padding: '2.5rem 2rem'}}>
                ...
                {isEditingProfile && (
                    <div style={{
                        ...
                        <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                            ...
                            >
                                {saving ? 'ΓÅ│ Saving...' : '≡ƒÆ╛ Save Changes'}
                            </button>
                        </div>
                    </div>
                )}
                ...
                {toast.show && (
                    <div className={`mentor-toast ${toast.type}`}>
                        {toast.type === 'success' ? 'Γ£à' : 'Γ¥î'} {toast.message}
                    </div>
                )}
                                ...
                                <div key={booking.id} className={`booking-card status-${booking.status.toLowerCase()}`}>
                                    <div className="booking-card-top">
                                        <div className="guest-info">
                                            ...
                                            <div className="guest-meta">
                                                <span>≡ƒôº {booking.guestEmail}</span>
                                                <span>≡ƒô▒ {booking.guestWhatsapp}</span>
                                            </div>
                                        </div>
                                    ...
                                    <div className="booking-details-grid">
                                        ...
                                        <div className="b-detail">
                                            ...
                                            <p>≡ƒôà {booking.bookingDate}</p>
                                        </div>
                                        <div className="b-detail">
                                            <label>Preferred Time</label>
                                            <p>ΓÅ░ {booking.bookingTime}</p>
                                        </div>
                                        <div className="b-detail">
                                            <label>Requested At</label>
                                            <p>ΓÅ▓∩╕Å {new Date(booking.createdAt).toLocaleString()}</p>
                                        </div>
                                        {booking.meetLink && (
            ...
            {showAnalytics && (
                <div className="analytics-section">
                    <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem'}}>
                        <h2 style={{fontSize: '2rem', fontWeight: 800, margin: 0, color: '#0f172a'}}>Dashboard Analytics</h2>
                        <div style={{display: 'flex', alignItems: 'center', gap: '10px'}}>
                            <span style={{fontSize: '0.9rem', color: '#64748b', fontWeight: 600}}>Time Period:</span>
                            <select 
                                value={viewFilter} 
                    ...
                    <div className="stats-grid">
                        <div className="stat-card">
                            <h3>{viewFilter === 'all' ? (profile.profileViews || 0) : getTotalViews()}</h3>
                            <p>Profile Views</p>
                        </div>
                        <div className="stat-card">
                        ...
                        <div className="stat-card">
                            <h3 style={{color: '#16a34a'}}>{bookings.filter(b => b.status === 'APPROVED' || b.status === 'SCHEDULED' || b.status === 'COMPLETED').length}</h3>
                            <p>Approved Requests</p>
                        </div>
                        <div className="stat-card">
                            <h3 style={{color: '#ef4444'}}>{bookings.filter(b => b.status === 'REJECTED').length}</h3>
                    ...
                    <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '2rem', marginTop: '2rem'}}>
                        ...
                        <div style={{background: 'white', padding: '2.5rem', borderRadius: '24px', border: '1px solid #f1f5f9'}}>
                            <h3 style={{marginBottom: '1.5rem', fontSize: '1.3rem'}}>Overall Performance</h3>
                            <div style={{display: 'flex', gap: '3rem', flexWrap: 'wrap'}}>
                                <div>
                                    <span style={{display: 'block', fontSize: '1.8rem', fontWeight: 800, color: '#4f46e5'}}>
                                        {bookings.length > 0 ? ((bookings.filter(b => b.status === 'APPROVED' || b.status === 'SCHEDULED' || b.status === 'COMPLETED').length / bookings.length) * 100).toFixed(1) : 0}%
                                    </span>
                                    <span style={{color: '#64748b', fontSize: '0.9rem', fontWeight: 600}}>Approval Rate</span>
                                </div>
                                <div>
                                    <span style={{display: 'block', fontSize: '1.8rem', fontWeight: 800, color: '#4f46e5'}}>
                                        {todayReceived}
                                    </span>
                                    <span style={{color: '#64748b', fontSize: '0.9rem', fontWeight: 600}}>Received Today</span>
                                </div>
                                <div>
                                    <span style={{display: 'block', fontSize: '1.8rem', fontWeight: 800, color: '#4f46e5'}}>
                                        {profile.rating?.toFixed(1) || '5.0'}
                                    </span>
                                    <span style={{color: '#64748b', fontSize: '0.9rem', fontWeight: 600}}>Avg. Rating</span>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Day-wise Records */}
                        <div style={{background: 'white', padding: '2.5rem', borderRadius: '24px', border: '1px solid #f1f5f9'}}>
                            <h3 style={{marginBottom: '1.5rem', fontSize: '1.3rem'}}>Day-wise View Records</h3>
                            <div style={{maxHeight: '300px', overflowY: 'auto', paddingRight: '10px'}}>
                                {getFilteredViews().length > 0 ? (
                                    getFilteredViews().map(date => (
                                        <div key={date} style={{
                                            display: 'flex', 
                                            justifyContent: 'space-between', 
                                            alignItems: 'center',
                                            ...
                                            <span style={{fontWeight: '600', color: '#334155'}}>{new Date(date).toLocaleDateString(undefined, {weekday: 'short', month: 'short', day: 'numeric'})}</span>
                                            <span style={{
                                                background: '#f1f5f9', 
                                                padding: '4px 12px', 
                                                borderRadius: '20px',
                                                ...
                                                fontWeight: '800',
                                                color: '#4f46e5'
                                            }}>{dailyViews[date]} views</span>
                                        </div>
                                    ))
                                ) : (
                                    <p style={{textAlign:'center', color:'#94a3b8', marginTop:'2rem'}}>No views recorded in this period.</p>
                                )}
                            </div>
                        </div>
                    </div>
                ...
                <div className="mentor-preview" style={{ margin: '0 auto', maxWidth: '1200px', width: '100%' }}>
                    ...
                    <div className="preview-body">
                        <div className="preview-top-row" style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', width: '100%', marginBottom: '1rem' }}>
                            <div className="preview-avatar-wrapper" style={{ marginBottom: 0 }}>
                                ...
                                {isEditingProfile ? (
                                    ...
                                    >
                                        {profile.isAvailable ? 'ΓÜí Available' : '≡ƒÜ½ Unavailable'}
                                    </button>
                                ) : (
                                    profile.isAvailable && <div className="availability-badge">ΓÜí Available</div>
                                )}
                            </div>
                            ...
                            <div className="preview-header-meta" style={{ 
                                ...
                                <span className="preview-rating" style={{ 
                                    ...
                                    Γ¡É {profile.rating || 'New'}
                                </span>
                                {isEditingProfile && <button className="inline-edit-btn" style={{position:'static'}} onClick={() => openEdit('socials', 'Social Links')}>Γ£Å∩╕Å</button>}
                                <div className="preview-socials" style={{ gap: '6px', display: 'flex', alignItems: 'center' }}>
                                    {profile.email && <a href={`mailto:${profile.email}`} className="preview-social-icon-raw">Γ£ë∩╕Å</a>}
                        ...
                        <div className="preview-title-row">
                            <h3>
                                {profile.name}
                                {isEditingProfile && <button className="inline-edit-btn" style={{position:'static', marginLeft:'10px'}} onClick={() => openEdit('name', 'Display Name', 'text')}>Γ£Å∩╕Å</button>}
                            </h3>
                        </div>
                        ...
                        <div className="preview-subtitle">
                            ...
                            {isEditingProfile && (
                                <div className="edit-buttons-stack">
                                    <button className="inline-edit-btn" style={{position:'static'}} onClick={() => openEdit('role', 'Job Role', 'text')}>Γ£Å∩╕Å Role</button>
                                    <button className="inline-edit-btn" style={{position:'static'}} onClick={() => openEdit('company', 'Company Name', 'text')}>Γ£Å∩╕Å Company</button>
                                </div>
                            )}
                        </div>

                        <div className="preview-badges">
                            <div className="preview-badge" style={{background: '#f8fafc', border: '1px solid #e2e8f0', color: '#64748b'}}>≡ƒÆ╝ {profile.experience ? (profile.experience.toLowerCase().includes('year') ? profile.experience : `${profile.experience} Years of experience`) : 'Experience'}</div>
                            {isEditingProfile && <button className="inline-edit-btn-mini" style={{position:'static', marginLeft:'12px'}} onClick={() => openEdit('experience', 'Experience', 'text')}>Γ£Å∩╕Å</button>}
                        </div>
                            ...
                            <div className={`preview-section-left ${mainTab === 'About' ? 'tab-visible' : 'tab-hidden'}`}>
                                ...
                                <div className="preview-accordion">
                                    <div className="preview-accordion-header" onClick={() => toggleAccordion('about')} style={{cursor: 'pointer'}}>
                                        ...
                                        <span className="accordion-action-wrapper">
                                            {isEditingProfile && <button className="inline-edit-btn-mini" onClick={(e) => { e.stopPropagation(); openEdit('bio', 'About', 'textarea'); }}>Γ£Å∩╕Å</button>}
                                ...
                                <div className="preview-accordion">
                                    <div className="preview-accordion-header" onClick={() => toggleAccordion('topics')} style={{cursor: 'pointer'}}>
                                        Topics of Expertise 
                                        <span className="accordion-action-wrapper">
                                            {isEditingProfile && <button className="inline-edit-btn-mini" onClick={(e) => { e.stopPropagation(); openEdit('topics', 'Topics of Expertise', 'text'); }}>Γ£Å∩╕Å</button>}
                                ...
                                <div className="preview-accordion">
                                    <div className="preview-accordion-header" onClick={() => toggleAccordion('education')} style={{cursor: 'pointer'}}>
                                        Education 
                                        <span className="accordion-action-wrapper">
                                            {isEditingProfile && <button className="inline-edit-btn-mini" onClick={(e) => { e.stopPropagation(); openEdit('education', 'Education', 'textarea'); }}>Γ£Å∩╕Å</button>}
                                ...
                                <div className="preview-accordion">
                                    <div className="preview-accordion-header" onClick={() => toggleAccordion('work')} style={{cursor: 'pointer'}}>
                                        Work Experience 
                                        <span className="accordion-action-wrapper">
                                            {isEditingProfile && <button className="inline-edit-btn-mini" onClick={(e) => { e.stopPropagation(); openEdit('workExperience', 'Work Experience', 'textarea'); }}>Γ£Å∩╕Å</button>}
                                ...
                                <div style={{background: '#f8fafc', padding: '1rem', borderRadius: '12px', border: '1px solid #e2e8f0'}}>
                                    ...
                                    {(activeServiceTab === 'All' || activeServiceTab === '1:1 Call') && (
                                        <div className="preview-service-card" style={{borderColor: '#bfdbfe'}}>
                                            <div className="preview-service-tag">Γ¡É BEST SELLER</div>
                                            <div className="preview-service-title">1:1 Call Mentorship</div>
                                            <div className="preview-service-footer">
                                                <div className="preview-service-price">Γé╣499</div>
                                                <button 
                                    ...
                                    {(activeServiceTab === 'All' || activeServiceTab === 'Resume Review') && (
                                        <div className="preview-service-card">
                                            <div className="preview-service-tag" style={{background: '#e0e7ff', color: '#4338ca'}}>≡ƒô¥ FEEDBACK</div>
                                            <div className="preview-service-title">Resume Review</div>
                                            <div className="preview-service-footer">
                                                <div className="preview-service-price">Γé╣199</div>
                                                <button 
                                                    className="preview-book-btn" 
            ...
            {showPreviewModal && (
                <div className="preview-modal-overlay" onClick={() => setShowPreviewModal(false)}>
                    <div className="preview-modal-content" onClick={(e) => e.stopPropagation()}>
                        <button className="preview-modal-close" onClick={() => setShowPreviewModal(false)}>Γ£ò</button>
                        ...
                        <div className="preview-body">
                            ...
                            <div className="preview-title-row" style={{marginBottom: '0.2rem'}}>
                                ...
                                <div className="preview-badges">
                                <div className="preview-badge" style={{background: 'white', border: '1px solid #e2e8f0', color: '#64748b', boxShadow: '0 1px 2px rgba(0,0,0,0.05)'}}>≡ƒÆ╝ {profile.experience ? (profile.experience.toLowerCase().includes('year') ? profile.experience : `${profile.experience} Years of experience`) : '4 Years of experience'}</div>
                                </div>
                                ...
                                <div className="preview-content-grid">
                                    ...
                                    <div className={`preview-section-right ${mainTab === 'Services' ? 'tab-visible' : 'tab-hidden'}`}>
                                        <h4 style={{color: '#1e1b4b'}}>≡ƒôà Available Services</h4>
                                        <div style={{background: '#f8fafc', padding: '1rem', borderRadius: '12px', border: '1px solid #e2e8f0'}}>
                                            <div style={{display: 'flex', background: '#e2e8f0', borderRadius: '8px', padding: '0.25rem', marginBottom: '1rem'}}>
                                                {['All', '1:1 Call', 'Resume Review'].map(tab => (
                                                    <button 
